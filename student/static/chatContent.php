...
while($msg = $messages->fetch_assoc()){
    if($friend_id_c != $msg['receive_id']){
        ...
        while($i = $individual_friend_list->fetch_assoc()){
            ...
            // }
            ?> 
                <li class="<?php if($sender_id == $student_id) echo 'left'; else echo 'right'?>">
    ...
    <div class="input-group">
        <form action="" method="post">
            ...
            <input type="hidden" value="" name="sender_id" />
            <input type="hidden" value="" name="receiver_id" />
            <span class="input-group-btn">
            <button class="btn btn-default" type="send-message">Send</button>
            </span>
        </form>
    </div>
    </div>
